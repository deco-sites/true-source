...
function HighlightsBanner({ title, cards }: Props) {
  return (
    <div class="w-full max-w-[1440px] mx-auto flex flex-col gap-8 bg-ice p-4 md:p-10 rounded-[20px]">
      ...
      <div class="flex flex-col items-center md:flex-row gap-y-4 justify-between">
        {cards?.map(({ image, tag, productName, productDescription, url }) => (
          <a href={url} class="relative">
            <Image
              ...
              width={660}
              height={450}
              class="rounded-[20px] h-full w-full md:max-w-[660px] md:max-h-[450px] object-cover hidden md:block"
            />
            <Image
              src={image.mobileSrc}
              alt={image.alt}
              width={358}
              height={217}
              class="rounded-[20px] h-full w-full max-w-[358px] max-h-[217px] object-cover md:hidden"
            />
