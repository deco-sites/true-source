...
export function Socials({ socials }: { socials: Socials[] }) {
  return (
    <div class='flex gap-[20px] p-6 mt-[76px]'>
      {socials.map((item) => (
        <a href={item.url} class='border border-[#E9530E] rounded-full w-[38.5px] h-[38.5px] flex items-center justify-center'>
          <Image src={item.icon} alt={item.name} width={16} height={16} class='max-w-4 max-h-4' />
        </a>
      ))}
    </div>
  );
...
export function MenuInstitutional({ institutionalItems }: { institutionalItems: InstitucionalItem[] }) {
  return (
    <div class='flex flex-col gap-6 mt-12'>
      {institutionalItems.map((item) => (
        <a href={item.url} class='font-lemon-milk text-[11px] text-dark px-6 uppercase font-medium'>
          {item.name}
        </a>
      ))}
    </div>
      ...
      <div class="flex flex-col">
        {itemsWithChildren.map((navItem, index) => (
          <Collapsable
            class="w-full"
            title={
              <div class={`flex relative justify-between py-[15.5px] font-lemon-milk text-[11px] text-dark border border-Stroke px-6 uppercase font-bold 
                ${index === 0 && "text-white bg-gradient-to-r from-[#E4003F] from-35% to-[#e8530e] to-90% rounded-full group-open:fontWithGradient group-open:border-red"}
              ...
              before:border-Stroke before:border-r before:absolute before:right-[-1px] before:top-1/2 before:h-[30px] before:-z-10
                ${index === 1 && 'border-t-0'}`}>
                <span class={`${navItem.ishighlighted && "fontWithGradient"} `}>{navItem.name}</span>
                <Icon id="ChevronDown"
                  size={16}
        ...
        {itemsWithoutChildren.map((item) => (
          <span class='py-[15.5px] font-lemon-milk text-[11px] text-dark border border-Stroke px-6 uppercase font-bold last:rounded-b-[8px]'>{item.name}</span>
        ))}
      </div>
